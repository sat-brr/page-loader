...
def test_download_pics():
    with requests_mock.Mocker() as m:
        ...
        paths = (f'tests/fixtures/pic1.png',
                 f'tests/fixtures/pic2.jpg',
                 f'tests/fixtures/pics/pic3.png')
        for path in paths:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            new_dir = f'{tmp_dir}/{make_path(TEST_URL, dir=True)}'
            for path in paths:
                local_path = f'{make_path(TEST_URL)}-{make_path(path, file=True)}'
...
def test_download_link_scripts():
    with requests_mock.Mocker()as m:
        ...
        m.get(TEST_URL, text=test_page)
        paths = (f'tests/fixtures/links/link1.css',
                 f'tests/fixtures/scripts/script1.js',)
        for path in paths:
            with open(path, 'rb') as file:
                tmp = file.read()
            m.get(f'{TEST_URL}{path}', content=tmp)
